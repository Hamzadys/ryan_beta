...
//	display_error($total_label);
	...
	}
	{
		array_append($th, array( _("Ordered"), _("Units"), _("Received"),
			_("Outstanding"), _("This Delivery"),  _("Total")) );
	}
...
/*elseif ($SysPrefs->hide_prices_grn() == 1)
    ...
    if (count($_SESSION['PO']->line_items)> 0 )
    {
       	foreach ($_SESSION['PO']->line_items as $ln_itm)
       	{
			...
			text_cells(null, $ln_itm->line_no."batch", $ln_itm->batch, 30, 50);
			...
			if($myrow_1['purchase_enable'])
			{
				...
				elseif($myrow_1['type'] == 4)
					text_cells(null, $myrow_1['name'], $ln_itm->$myrow_1['name'], 20, 60);
			}
			if($myrow_2['purchase_enable'])
			{
				if($myrow_2['type'] == 1)
			...
			{
				if($myrow_3['type'] == 1)
					amount_cells(null, $myrow_3['name'], $ln_itm->$myrow_3['name']);
	...
	foreach ($_SESSION['PO']->line_items as $line)
	{
	 if( ($line->quantity - $line->qty_received)>0) {
		...
		if (!isset($_POST['DefaultReceivedDate']) || $_POST['DefaultReceivedDate'] == "")
		 ...
		 $_SESSION['PO']->line_items[$line->line_no]->batch = $_POST[$line->line_no.'batch'];
         $_SESSION['PO']->line_items[$line->line_no]->text1 = $_POST[ "text1"];
