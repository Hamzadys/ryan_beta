...
<?php
	class renderer
	{
		...
		function menu_header($title, $no_menu, $is_index)
		{
			...
			global $path_to_root, $help_base_url,$img,$img2,$img3,$img4, $systypes_array, $db_connections;
...
include_once("$path_to_root/themes/".user_theme()."/Datamodal.php");                    
 ...
 echo'</div>';
      ...
      // images for notification icons
        ...
        $img4 = "$path_to_root/themes/".user_theme()."/images/Apps-Help-icon.png"; 
...
//              <!-- logo for regular state and mobile devices -->
        ...
        <!-- Setups -->
    		...
    		$sql = "SELECT a.*,
		            ...
		            ORDER BY a.stamp DESC LIMIT 20
    		    	";
    		$result = db_query($sql);
...
/*            echo"
                <li>
                    ...
                    _('Amount') .' <font color="red">'. 
                      number_format2($myrow['amount'], 0) .' </font>'. 
                      $myrow['description'] .' '.
                      _('by') .' <i>'.
                      $myrow['user_id'] .' </i>'. 
                      _('at') .' '.
                        ...
                        //var_dump($app);
                          ...
                          if($app->name =="Dash&Board")
                            ...
                            <a style='' class='"
                             ...
                             else  if($app->name =="&Items and Inventory")
                            {
                                $gly = "fa fa-barcode";
                            $leftmenu_save .= "      <li class='treeview'>";
                            $leftmenu_save .= "
                            <a style='' class='"
                             ...
                             else  if($app->name =="&Manufacturing")
                            {
                                $gly = "fa fa-industry";
                            $leftmenu_save .= "      <li class='treeview'>";
                            $leftmenu_save .= "
                            ...
                            <a style='' class='"
                             ...
                             else if($app->name =="&Banking and General Ledger")
                            {
                                $gly = "fa fa-bar-chart";
		...
		function renderButtonsForAppFunctions($appfunction)
		{
			...
			if ($_SESSION["wa_current_user"]->can_access_page($appfunction->access))
			{
				if ($appfunction->label != "")
				{
			        ...
			        if($appfunction->link == 'purchasing/manage/suppliers.php?')
        			{
            			$badge = $supp_count;
                        $bgcolor = 'bg-gray';
            			}       
            		if($appfunction->link == 'inventory/manage/items.php?')
        			{
            			$badge = $stock_count;
                        $bgcolor = 'bg-gray';
            			}     
