...
$th = array(_(""),_("Req#"), _("Point of Use"), _("Request Date"),_("Item Code"), _("Item Name"),
	_("Purpose"), _("Quantity"), _("Price"),  _("Supplier"), "",_("Print"));
...
//if ($id != -1)
{
	...
	update_make_po($id, $reconcile_value);
}
//display_error(check_value('rec_'.$id));


function update_make_po($id, $make_po)
{
	$TotalRecord = get_count_requisition_details();
//	display_error($TotalRecord['TotalRecord']);
	for($i=1; $i <= $TotalRecord['TotalRecord']; $i++)
	{
		$reconcile_value = check_value("rec_".$i)
			? '1' : '0';
